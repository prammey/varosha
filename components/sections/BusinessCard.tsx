...
export function BusinessCard({ site }: { site: SiteDetails }) {
  return (
    <div className="relative z-[1] mx-auto grid max-w-[760px] overflow-hidden rounded-card bg-cream shadow-[0_40px_80px_-30px_rgba(0,0,0,.7)] transition-transform duration-300 sm:grid-cols-[1fr_1.4fr] md:-rotate-[1.2deg] md:hover:rotate-0">
      <div className="relative flex flex-col items-start justify-center gap-4 overflow-hidden bg-maroon px-[30px] py-9 text-on-plum after:absolute after:-bottom-[60px] after:-right-[60px] after:h-[180px] after:w-[180px] after:rounded-full after:bg-gold after:opacity-25">
        ...
        <span className="display text-[2rem] leading-none" style={{ fontVariationSettings: '"SOFT" 80, "WONK" 1' }}>{brand.name}</span>
        <span className="font-sans text-[0.78rem] font-semibold uppercase tracking-[0.1em] text-gold-soft">{brand.tagline}</span>
      </div>
      ...
      <div className="flex flex-col justify-center gap-3.5 px-[34px] py-9">
        ...
        <Row label="Social">
          <a href={site.facebook} target="_blank" rel="noopener" className="font-sans font-bold">{site.facebook.replace(/^https?:\/\/(www\.)?/, "")}</a>
        </Row>
        <Row label="Mail">
          <address className="not-italic leading-[1.45] text-ink-2">
            {site.addressName}<br />{site.street}<br />{site.cityStateZip}
          </address>
        </Row>
