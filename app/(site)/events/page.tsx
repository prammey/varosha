            ...
            <Reveal>
              <div className="relative pb-6 pr-6 after:absolute after:bottom-0 after:right-0 after:z-0 after:h-4/5 after:w-4/5 after:-rotate-2 after:rounded-card after:bg-rose-2">
                <div className="relative z-[1] aspect-[3/2] w-full overflow-hidden rounded-card">
                  <Image src={current.image} alt={current.imageAlt} fill unoptimized={current.image.startsWith("http")} sizes="(max-width: 860px) 100vw, 50vw" className="object-cover" />
                </div>
              </div>
            </Reveal>
          )}
          <Reveal delay={1} className="flex max-w-[64ch] flex-col gap-3.5">
            <span className="eyebrow text-maroon">{current.date}</span>
            <h2 className="text-[clamp(2rem,3.8vw,3.1rem)]">{current.title}</h2>
            {current.summary.split(/\n\s*\n/).map((p) => (
              <p key={p.slice(0, 40)} className={p.startsWith("*") ? "font-sans text-[0.95rem] text-ink-2 whitespace-pre-line" : undefined}>{p}</p>
            ))}
          </Reveal>
        </Container>
      )}
              ...
              <Reveal key={e.date + e.title} delay={(i % 2) as 0 | 1}>
                <article className="group flex h-full flex-col overflow-hidden rounded-card bg-white transition-[transform,translate,rotate,scale,box-shadow] duration-300 hover:-translate-y-1.5 hover:shadow-card">
                  <div className="relative aspect-[3/2] overflow-hidden">
                    <Image src={e.image!} alt={e.imageAlt} fill unoptimized={e.image!.startsWith("http")} sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transition-transform duration-500 group-hover:scale-[1.03]" />
                  </div>
                  <div className="flex flex-1 flex-col gap-2.5 px-6 pb-[26px] pt-[22px]">
                    ...
                    <p className="text-base text-ink-2">{e.summary.split(/\n\s*\n/)[0]}</p>
                  </div>
                </article>
              </Reveal>
            ))}
