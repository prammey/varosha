...
import { getNewsletters, getSiteDetails } from "@/lib/data";

export const metadata: Metadata = {
  title: "Newsletters",
  description: "Varosha's annual newsletters, as PDFs.",
};

export default async function NewslettersPage() {
  const [newsletters, site] = await Promise.all([getNewsletters(), getSiteDetails()]);
  const latestNewsletter = newsletters[0];
  const years = newsletters.length;
  return (
    <div className="bg-plum text-on-plum">
      <PageHeader
        tone="dark"
        eyebrow="Newsletters"
        title={<>{years === 9 ? "Nine" : years} years, <em className="text-gold">in print</em>.</>}
